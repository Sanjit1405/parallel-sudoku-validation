...
// for outputting the buffer to the file safely
pthread_mutex_t output_mutex; 
// for TAS implementation
...
int main(){
	...
	if(invalid){
		output_file << "Sudoku is invalid" << '\n';
		cout << "Sudoku is invalid" << '\n';
	}
	else{
		output_file << "Sudoku is valid" << '\n'; 
...
void* thread_function(void* arg){
    ...
    while(true){
		...
		// Allocate the tasks based on the counter incremented
		if(N%taskInc == 0){
