...
void *row_check(void * arg){
	...
	while(row < N){
		...
		for (int i = 0; i < N; i++){
			if(tracker[p->sudoku[row][i]-1] == 1){ // if there is a duplicate stop checking that row 
				flag = 0;
				break;
			}
		...
		row+=K1; // the thread validates in a cyclic manner
	}
	return NULL;
}
void *col_check(void * arg){
	...
	while(col < N){
		...
		int tracker[100] = {0}; // array to store 1 or 0, 1 if a number is there 0 if its not
		for (int i = 0; i < N; i++){
			if(tracker[p->sudoku[i][col]-1] == 1){ // if there is a duplicate stop checking that column 
				flag = 0;
				break;
			}
		...
		col+=K2; // the thread validates in a cyclic manner
	}
	return NULL;
}
void *subGrid_check(void * arg){
	...
	while(subGrid < N){
		...
		for (int i = n * (subGrid/n); i < n * (subGrid/n) + n; i++){
			for (int j = n * (subGrid % n); j < n * (subGrid % n) + n; j++){
				if(tracker[p->sudoku[i][j]-1] == 1){ // if there is a duplicate stop checking that subgrid 
					flag = 0;
					break;
				}
		...
		subGrid+=K3; // the thread validates in a cyclic manner
	}
	return NULL;
}